...
| 2.6 | 21 keyword rules | [adaptations.ts:304-332](../src/constants/adaptations.ts#L304) | "kettlebell swing is power", "pogo is power", etc. — a physiological classification carrying **no digit**. All point at power: the four sprint/reactive rules (`sprint`, `dash`, `agility`, `pogo`) tagged the retired `speed` adaptation until 2026-09-01 and were repointed with it ([done/019](roadmap/done/019-adaptation-model-simplification.md)). Since 2026-09-03 `hop` / `jump` match whole words only (a "Cable Woodchop" set was silently reading as power), `clapping` is on the list and "jump rope" is excluded | named | grounded (17 keywords supported; `sled` partially; `agility` convention; the rule's exclusivity is a decision — D16, 2026-09-03) | [039 S4](roadmap/039-adaptations-read-grounding.md#grounding) |
