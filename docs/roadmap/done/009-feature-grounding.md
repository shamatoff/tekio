...
**Label:** infra
**Status:** done — 2026-09-01. Deliverables shipped and acceptance all ticked. The lint/dead-code/perf half lives in 023; the back-fill remainder is tracked by its own briefs (013 for cycle/deload, 001/005 for the 3.4/3.8 cues), and item 2 (recovery weights) closed differently — RECOVERY_WEIGHTS was retired, not reweighted (see 014).
...
| 5 | Maintainability | existing `/simplify` + `/code-review`, plus **add ESLint + knip** and a tekio conventions file | **moved out to [023](../023-mechanical-code-quality-tooling.md)** | tooling only |
| 6 | Performance | `npm run perf` budget script; an agent interprets the numbers | **moved out to [023](../023-mechanical-code-quality-tooling.md)** | script, not agent |
...
| 2 | **§4 recovery weights** (4.6–4.10) — the Habits reweight touches them, though per §13.7 the reweight itself does not ground them. Rows 4.11 and 4.12 belong in the same brief: they decide what the readiness number *means*. | [010-home-fused-reads.md](010-home-fused-reads.md) | **retired 2026-08-31** — `RECOVERY_WEIGHTS` was deleted, not reweighted (fused Home; see [014](014-doctrine-ledger-execution.md)); rows 4.6–4.9 retired with the constant |
| 3 | **§5 cycle & deload** — the disagreements (four cycle-length copies saying `6, 6, 6, 4`; two deload models one screen apart) were bugs findable without research, and were fixed 2026-08-26. The claim itself is still untouched. | [013-cycle-deload-grounding.md](../013-cycle-deload-grounding.md) — **created 2026-08-26**, closing the last `(no brief)` domain in the inventory | pending — tracked in 013 |
| 4 | **3.4 / 3.8 cues** — the known debt, plus its twin. Cheapest possible run: both are named protocols, and the only question is who said it and on what. | [001-cross-adaptation-rep-ranges.md](001-cross-adaptation-rep-ranges.md) (3.4) · [005-hr-zone-intensity-classification.md](../005-hr-zone-intensity-classification.md) (3.8) | pending — tracked in 001 / 005 |

**Superseded 2026-08-31 — item 2 is closed:** the weights were retired, not
reweighted (see the readiness comparison in
[014](014-doctrine-ledger-execution.md)), so no run is coming and none is
needed. Kept below as the record of why the run would never have fired on its
own: the Habits reweight is `/ground` exemption 1
(renormalisation) by construction, so it does not fire the trigger — see
[015-ground-trigger-spec-fixes.md](../015-ground-trigger-spec-fixes.md) §13.7. Either apply
that brief's proposed fix (exemption 1 requires a `grounded` / `convention` base)
or accept that R2's 6-week clock is the only thing that will ever pull the
recovery weights in.
...
- [x] A feature like "adaptive deload frequency" trips the numeric trigger, `/ground`
...
- [x] The scout never edits files.
- [x] The doctrine is short enough to be read every time and specific enough to say
